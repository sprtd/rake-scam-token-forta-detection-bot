...
describe("Rake Scam Token Test Suite", () => {
  ...
  it("should ignore non fee on transfer function call on Uniswap's Router contract", async () => {
    const pair = uniCreate2(TEST_CASES.WETH, TEST_CASES.SCAM_TOKEN_1, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
  ...
  describe("SwapExactETHForTokensSupportingFeeOnTransferTokens", () => {
    it.only("should return correct finding when swapExactETHForTokensSupportingFeeOnTransferTokens function is called on Uniswap's Router contract", async () => {
      const pair = uniCreate2(TEST_CASES.WETH, TEST_CASES.SCAM_TOKEN_1, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const [amount0In, amount1Out] = ["5000000", "3000"];
    ...
    it.only("should return correct finding when swapExactETHForTokensSupportingFeeOnTransferTokens function is called on Uniswap's Router contract and raked fee is transferred to an EOA", async () => {
      const pair = uniCreate2(TEST_CASES.WETH, TEST_CASES.SCAM_TOKEN_2, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const [amount0In, amount1Out] = ["5000000", "3000"];
    ...
    it.only("should return findings when swapExactETHForTokensSupportingFeeOnTransferTokens function with more than 2 paths is called on Uniswap's Router contract", async () => {
      const pair1 = uniCreate2(TEST_CASES.WETH, TEST_CASES.SCAM_TOKEN_3, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair2 = uniCreate2(TEST_CASES.SCAM_TOKEN_3, TEST_CASES.TOKEN_2, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair3 = uniCreate2(TEST_CASES.TOKEN_2, TEST_CASES.SCAM_TOKEN_4, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
    ...
    it.only("should ignore swapExactETHForTokensSupportingFeeOnTransferTokens function calls on Uniswap's Router contract with  insignificant raked fee percentage", async () => {
      const pair = uniCreate2(TEST_CASES.WETH, TEST_CASES.SCAM_TOKEN_1, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash,);
      const [amount0In, amount1Out] = ["5000000", "3000"];
    ...
    it.only("should return findings when  swapExactETHForTokensSupportingFeeOnTransferTokens function containing multiple paths is called on Uniswap's Router contract", async () => {
      const pair1 = uniCreate2(TEST_CASES.WETH, TEST_CASES.TOKEN_1, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair2 = uniCreate2(TEST_CASES.TOKEN_1, TEST_CASES.TOKEN_2, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair3 = uniCreate2(TEST_CASES.TOKEN_2, TEST_CASES.TOKEN_3, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair4 = uniCreate2(TEST_CASES.TOKEN_3, TEST_CASES.SCAM_TOKEN_5, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair5 = uniCreate2(TEST_CASES.SCAM_TOKEN_5, TEST_CASES.TOKEN_4, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
  ...
  describe("SwapExactTokensForETHSupportingFeeOnTransferTokens", () => {
    it.only("should return finding when swapExactTokensForETHSupportingFeeOnTransferTokens function is called on Uniswap's Router contract", async () => {
      const functionName = "swapExactTokensForETHSupportingFeeOnTransferTokens";
      const pair = uniCreate2(TEST_CASES.SCAM_TOKEN_6, TEST_CASES.WETH, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const [amount0In, amount1Out] = ["5000000", "3000"];
    ...
    it.only("should return finding when swapExactTokensForETHSupportingFeeOnTransferTokens function is called on Uniswap's Router contract and raked fee is transferred to an EOA", async () => {
      const functionName = "swapExactTokensForETHSupportingFeeOnTransferTokens";
      const pair = uniCreate2(TEST_CASES.SCAM_TOKEN_7, TEST_CASES.WETH, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const [amount0In, amount1Out] = ["5000000", "3000"];
      const rakedFeePercentage = 10;
      const actualAmount = takeFee(toBn(amount0In), toBn(rakedFeePercentage));
    ...
    it.only("should ignore swapExactTokensForETHSupportingFeeOnTransferTokens function calls on Uniswap's Router contract with  insignificant raked fee percentage", async () => {
      const functionName = "swapExactTokensForETHSupportingFeeOnTransferTokens";
      const pair = uniCreate2(TEST_CASES.SCAM_TOKEN_8, TEST_CASES.WETH, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const [amount0In, amount1Out] = ["5000000", "3000"];
    ...
    it.only("should return findings when swapExactTokensForETHSupportingFeeOnTransferTokens function with more than 2 path addresses is called on Uniswap's Router contract", async () => {
      const pair1 = uniCreate2(TEST_CASES.TOKEN_1, TEST_CASES.TOKEN_2, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair2 = uniCreate2(TEST_CASES.TOKEN_2, TEST_CASES.SCAM_TOKEN_9, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair3 = uniCreate2(TEST_CASES.SCAM_TOKEN_9, TEST_CASES.WETH, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
    ...
    it.only("should return multiple findings when swapExactETHForTokensSupportingFeeOnTransferTokens function with multiple path addresses is called on Uniswap's Router contract", async () => {
      const pair1 = uniCreate2(TEST_CASES.WETH, TEST_CASES.TOKEN_1, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair2 = uniCreate2(TEST_CASES.TOKEN_1, TEST_CASES.TOKEN_2, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair3 = uniCreate2(TEST_CASES.TOKEN_2, TEST_CASES.TOKEN_3, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair4 = uniCreate2(TEST_CASES.TOKEN_3, TEST_CASES.SCAM_TOKEN_10, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair5 = uniCreate2(TEST_CASES.SCAM_TOKEN_10, TEST_CASES.TOKEN_4, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
  ...
  describe("SwapExactTokensForTokensSupportingFeeOnTransferTokens", () => {
    it("should return correct finding when swapExactTokensForTokensSupportingFeeOnTransferTokens function is called on Uniswap's Router contract", async () => {
      ...
      const pair = uniCreate2(TEST_CASES.SCAM_TOKEN_1, TEST_CASES.TOKEN_1, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
    ...
    it("should return findings when swapExactTokensForTokensSupportingFeeOnTransferTokens function with more than 2 path addresses is called on Uniswap's Router contract", async () => {
      const pair1 = uniCreate2(TEST_CASES.TOKEN_1, TEST_CASES.TOKEN_2, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair2 = uniCreate2(TEST_CASES.TOKEN_2, TEST_CASES.SCAM_TOKEN_1, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair3 = uniCreate2(TEST_CASES.SCAM_TOKEN_1, TEST_CASES.TOKEN_3, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const rakedFeePercentage = 5;
      const [
        pair1Amount0In,
        pair1Amount1Out,
    ...
    it("should return multiple findings when swapExactTokensForTokensSupportingFeeOnTransferTokens function is called on Uniswap's Router contract with multiple scam token addresses in path", async () => {
      ...
      const pair2 = uniCreate2(TEST_CASES.TOKEN_2, TEST_CASES.SCAM_TOKEN_1, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair3 = uniCreate2(TEST_CASES.SCAM_TOKEN_1, TEST_CASES.TOKEN_3, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair4 = uniCreate2(TEST_CASES.TOKEN_3, TEST_CASES.TOKEN_4, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
      const pair5 = uniCreate2(TEST_CASES.TOKEN_4, TEST_CASES.SCAM_TOKEN_2, mockNetworkManager.factory, mockNetworkManager.pairInitCodeHash);
